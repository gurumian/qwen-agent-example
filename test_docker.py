#!/usr/bin/env python3
"""
Docker Testing Script for Qwen-Agent Chatbot
...
class DockerTester:
    def __init__(self):
        ...
        self.container_name = f"{self.project_name}-test"
        self.network_name = "qwen-agent-network"
        self.port = 8002
    ...
    def test_dockerfile(self) -> bool:
        ...
        try:
            result = self.run_command(["docker", "build", "--dry-run", "."])
            if result.returncode == 0:
                self.print_status("Dockerfile syntax is valid", "SUCCESS")
                return True
            else:
                self.print_status("Dockerfile syntax error", "ERROR")
                return False
        except Exception as e:
            self.print_status(f"Dockerfile test failed: {e}", "ERROR")
            return False
